...
wordVecs = lapply(speechWords, function(x){
  counts = table(x)
  temp = emptyVec
  temp[names(counts)] = counts #카운트꺼를 그냥 이 빈거에다가 처 넣는거고 그리고 temp[names(counts)]는 
  #카운트를 빈거에다가 넣기 위한 멍석이라고 보면된다. 그리고 이거의 핵심은 names(emptyVec)인듯.. 
  #벡터에 names를 붙여주면 그 벡터의 섭세팅[] 할때 그대로 그 엘레멘트쓰면 그거 찾아준다. 이게 핵심인듯. 
  return(temp)
})
